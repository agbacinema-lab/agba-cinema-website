      ...
      <header className="fixed top-0 inset-x-0 z-50 h-20 bg-black text-white flex items-center justify-between px-6 md:px-12 shadow-[0_4px_30px_rgba(0,0,0,0.5)] border-b border-white/5 backdrop-blur-md bg-black/90 transition-all">
        <div className="flex items-center gap-6">
           <div className="w-10 h-10 bg-yellow-400 rounded-xl flex items-center justify-center transform hover:rotate-12 transition-transform">
            ...
            <div>
              <p className="text-[10px] text-yellow-400 font-black uppercase tracking-[0.4em]">Admin Portal</p>
              <h1 className="text-xl font-black italic tracking-tighter leading-none capitalize text-white">
      ...
      <div className="flex pt-20 transition-colors">
        ...
        <aside className="hidden md:flex flex-col w-80 fixed top-20 bottom-0 left-0 bg-black text-white p-8 border-r border-white/5 overflow-y-auto custom-scrollbar">
          <div className="mb-10 px-4">
             ...
             <p className="text-[10px] font-black uppercase tracking-[0.5em] text-white/40">Operational Menu</p>
          </div>
          <nav className="flex-1 space-y-2">
            ...
            {isStaff && (
              <>
                ...
                <NavItem active={activeTab === 'readiness'} disabled={!hasAccess('readiness')} icon={<GraduationCap className="h-5 w-5" />} label="Internship Ready" onClick={() => setActiveTab('readiness')} />
        ...
        <main className="flex-1 md:ml-80 min-h-[calc(100vh-5rem)] overflow-y-auto transition-colors">
          ...
          <div className="p-8 md:p-16 pb-32 md:pb-20 max-w-7xl">
            <div className="hidden md:block mb-12 border-b border-muted pb-8">
              <h2 className="text-5xl lg:text-6xl font-black italic uppercase tracking-tighter text-foreground mb-4 transition-colors">Admin Hub: {profile?.name.split(' ')[0]}</h2>
              <p className="text-muted-foreground font-medium italic text-xl">Manage the entire ÀGBÀ CINEMA platform from one place.</p>
            </div>
...
function NavItem({ active, icon, label, onClick, disabled }: any) {
  return (
    <button
      ...
      disabled={disabled}
      className={`w-full flex items-center gap-5 p-5 rounded-[1.5rem] transition-all font-black text-[11px] uppercase tracking-[0.3em] italic ${
        disabled 
    ...
    >
      <div className={`transition-colors ${active ? 'text-black' : 'text-yellow-400/40'}`}>{icon}</div>
      {label}
    </button>
  )
}
...
function StatCard({ label, value, subtext, color }: any) {
  return (
    <Card className="border border-muted shadow-premium bg-card p-10 rounded-[3rem] hover:scale-[1.03] transition-all duration-500 overflow-hidden relative group">
      <div className={`w-14 h-14 rounded-2xl mb-8 flex items-center justify-center font-black text-xl italic shadow-xl transition-all group-hover:bg-yellow-400 group-hover:text-black ${color}`}>
        {value[0]}
      </div>
      <div>
         <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.4em] mb-2 transition-colors group-hover:text-yellow-500">{label}</p>
         <h3 className="text-4xl font-black text-foreground italic tracking-tighter leading-none mb-3 transition-colors">{value}</h3>
         <p className="text-[11px] text-muted-foreground font-medium italic opacity-60">{subtext}</p>
      </div>
