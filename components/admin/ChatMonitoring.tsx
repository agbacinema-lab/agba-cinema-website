...
import { MessageSquare, Shield, Search, Users, Clock, User, Zap } from "lucide-react"
...
export default function ChatMonitoring({ currentUser, title, subtitle }: ChatMonitoringProps) {
  ...
  useEffect(() => {
    ...
    // Super admins, directors and admins see ALL rooms
    const isSystemAdmin = ['super_admin', 'director', 'admin'].includes(currentUser?.role || '');
    // Tutors see rooms they are a participant in (their UID is stored directly)
    const isTutor = currentUser?.role === 'tutor';

    let searchIds: string[] | undefined;
    if (!isSystemAdmin) {
      searchIds = [currentUser.uid];
      // Students also search by their public studentId for backward compat
      if (currentUser.role === 'student' && (currentUser as any).studentId) {
        searchIds.push((currentUser as any).studentId);
      }
      // Tutors just use their UID (it's stored directly in participants)
    }
    ...
    return () => unsubscribe()
  }, [currentUser, currentUser?.role, currentUser?.uid, (currentUser as any).studentId])

  const filteredRooms = rooms.filter(r => {
    // Robust naming detection
    const sName = r.metadata?.studentName || r.participantDetails?.[r.metadata?.studentId]?.name || "Student";
    const bName = r.metadata?.brandName || r.participantDetails?.[r.metadata?.brandId]?.name || "Brand";
  ...
  return (
    <div className="space-y-8 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          <h2 className="text-3xl font-black uppercase tracking-tighter">{title || "Communications Hub"}</h2>
          <p className="text-muted-foreground font-medium mt-1 uppercase text-[10px] tracking-[0.2em]">{subtitle || "Official Liaison & Deployment Monitoring"}</p>
        </div>
        <div className="relative w-full md:w-96 group">
          <Search className="absolute left-6 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground group-focus-within:text-yellow-400 transition-colors" />
          <Input
        ...
        <div className="lg:col-span-4 space-y-4">
          {loading ? (
            <div className="p-12 text-center space-y-4">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Syncing chats...</p>
            </div>
            ...
            <div className="p-12 text-center border-2 border-dashed border-muted rounded-[3rem]">
              ...
              <p className="text-xs font-black uppercase tracking-widest text-muted-foreground italic">No active chat.</p>
            </div>
          ) : (
            filteredRooms.map((room) => {
              const isStudent = currentUser?.role === 'student';
              const sName = room.metadata?.studentName || room.participantDetails?.[room.metadata?.studentId]?.name || "Student";
              const bName = room.metadata?.brandName || room.participantDetails?.[room.metadata?.brandId]?.name || "Brand";
              
              const primaryName = isStudent ? bName : sName;
              const secondaryName = isStudent ? sName : bName;
              ...
              return (
                <motion.div
                  ...
                  className={`p-6 rounded-[2rem] border-2 cursor-pointer transition-all ${activeRoomId === room.roomId
                ...
                >
                  ...
                  <h4 className="text-xs font-black uppercase tracking-tighter mb-2 italic">
                    {isStudent ? "My Deployment" : `Ref: ${secondaryName}`}
                  </h4>
