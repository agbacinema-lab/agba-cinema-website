...
export default function CurriculumMaterials({
  ...
  }, [curriculumId, moduleId, materials])
    ...
    <div className="space-y-6 border-t pt-6">
      ...
      {showForm && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          <Card className="border border-muted shadow-premium rounded-2xl bg-card p-8 text-left">
            <div className="flex justify-between items-center mb-6 text-left">
              <h4 className="text-lg font-black tracking-tighter">Add learning material</h4>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-xs font-black tracking-widest text-gray-400">Material type</label>
                <div className="grid grid-cols-3 gap-3">
                  {(['pdf', 'video', 'link'] as const).map((type) => (
                    <button
                      ...
                      className={`h-12 rounded-xl font-black uppercase text-[10px] tracking-widest border-2 transition-all flex items-center justify-center gap-2 ${
                        materialType === type
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  className="min-h-20 rounded-xl border-muted-foreground/30 bg-background"
                />
              </div>
              ...
              {materialType === 'pdf' && (
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className="h-12 rounded-xl border-muted-foreground/30 bg-background"
                  />
                  <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 space-y-2 text-[10px] text-blue-400">
                    <p className="font-black tracking-widest uppercase">How to get your Google Drive link:</p>
                  ...
                  {formData.driveUrl && (
                    <div className="flex items-center justify-between gap-3 bg-green-500/10 border border-green-500/20 px-4 py-3 rounded-xl">
                      <div className="flex items-center gap-2 text-[10px] text-green-400 font-black tracking-widest uppercase">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 bg-green-600 hover:bg-green-700 text-white text-[10px] font-black tracking-widest px-3 py-1.5 rounded-lg transition-all"
                      >
              ...
              {materialType === 'video' && (
                <div className="space-y-2">
                  <label className="text-xs font-black tracking-widest text-gray-400">Video URL *</label>
                  <Input
              ...
              {materialType === 'link' && (
                <div className="space-y-2">
                  <label className="text-xs font-black tracking-widest text-gray-400">External link *</label>
                  <Input
      ...
      <div className="space-y-4">
        {localMaterials.length === 0 ? (
          <Card className="border border-muted shadow-sm rounded-xl bg-muted/20 p-8 text-center">
            <p className="text-muted-foreground font-medium">No materials added yet</p>
            <p className="text-[10px] text-muted-foreground/40 mt-1 font-black tracking-widest uppercase">Upload pdfs via Google Drive or add video/links</p>
          </Card>
        ) : (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 text-left">
                      <div className="flex items-center gap-2 mb-2 flex-wrap">
                        ...
                        <span className="text-[10px] bg-muted text-muted-foreground px-2 py-1 rounded-full font-black tracking-widest uppercase">
                          {getSourceBadge(material)}
                        </span>
                      </div>
                      <h4 className="font-black text-foreground mb-1 tracking-tighter">{material.title}</h4>
                      <p className="text-sm text-muted-foreground line-clamp-2 leading-relaxed">{material.description}</p>
                    </div>
